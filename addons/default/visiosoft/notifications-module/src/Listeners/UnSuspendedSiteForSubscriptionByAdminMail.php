...
class UnSuspendedSiteForSubscriptionByAdminMail
{
    ...
    public function handle(UnSuspendedSiteForSubscription $event)
    {
        $site = $event->getSite();

        if ($user = $site->assign) {
            $template = $this->template->findBySlug(str_slug('UnSuspended Site For Subscription By Admin', '_'));
