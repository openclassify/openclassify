...
class ClassifiedTableColumns
{
    ...
    public function handle(ClassifiedTableBuilder $builder)
    {
        $columns = [
            'cover_photo' => [
                'value' => function (EntryInterface $entry) {
                    return "<img width='80px' src='" . $entry->AddClassifiedsDefaultCoverImage($entry)->cover_photo . "' >";
                }
            ],
            ...
            'name' => [
                ...
                'value' => [
                    ...
                    'name' => function (EntryInterface $entry) {
                        if ($entry->getTitle()) {
                            $value = "<a href='" . $entry->getClassifiedDetailLinkByModel($entry, 'list') . "' > {entry.name}</a > ";
                        } else {
                            $value = "<font color='red'>" . trans("visiosoft.module.classifieds::view.unfinished") . "</font>";
                        }
                        return $value;
                    },
        ...
        if ($builder->isActiveView('classifiedanced')) {

            unset($columns['created_by'], $columns['country']);
            $columns['is_get_classified'] = [
                'attributes' => [
                    'html' => function (EntryModel $entry) {
                        $checked = ($entry->is_get_classified) ? 'checked' : '';
                        return '<input style="min-width:120px" type="checkbox" class="form-control fast-update" ' . $checked . ' data-column="is_get_classified" data-entry_id="' . $entry->getId() . '">';
                    }
                ],
                'class' => 'classifieds-price',
            ];
