...
class SubscriptionPaymentFailedForWebhookMail
{
    ...
    public function handle(SubscriptionFailedForWebhook $event)
    {

        if ($user = $event->getSubscription()->assign) {
            $subscription = $event->getSubscription();

            $template = $this->template->findBySlug(str_slug('Failed Payment Subscription For Paddle', '_'));
