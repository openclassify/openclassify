...
class OptionConfigurationController extends PublicController
{
    ...
    {
        $form->setOption('redirect', route('classifieds_preview', [request('classified')]));
        return $form->render();
    }
    ...
    public function confAddCart()
    {
        if ($conf = $this->optionConfigurationRepository->find($this->request->configuration)) {
            if ($conf->parent_classified->getStatus() == "approved") {
            ...
            $this->messages->info(trans('visiosoft.module.classifieds::message.error_added_cart'));
            return back();
        }
    }
