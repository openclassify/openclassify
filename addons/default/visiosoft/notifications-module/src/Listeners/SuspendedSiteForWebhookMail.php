...
class SuspendedSiteForWebhookMail
{
    ...
    public function handle(SuspendedSiteForWebhook $event)
    {
        $site = $event->getSite();

        if ($user = $site->assign) {
            $template = $this->template->findBySlug(str_slug('Suspended Site For Webhook', '_'));
