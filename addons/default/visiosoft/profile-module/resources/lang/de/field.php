<?php

return [
    'file' => [
        'name' => 'Fotos'
    ],
    'first_name' => [
        'name' => 'Vorname'
    ],
    'last_name' => [
        'name' => 'Nachname'
    ],
    'username' => [
        'name' => 'Nutzername'
    ],
    'display_name' => [
        'name' => 'Anzeigename'
    ],
    'email' => [
        'name' => 'E-Mail-Adresse'
    ],
    'sitename' => [
        'name' => 'Site-Name'
    ],
    'activated' => [
        'name' => 'Benutzer aktiv'
    ],
    'enabled' => [
        'name' => 'Benutzer aktivieren'
    ],
    'country' => [
        'name' => 'Land'
    ],
    'city' => [
        'name' => 'Stadt'
    ],
    'district' => [
        'name' => 'Kreis'
    ],
    'neighborhood' => [
        'name' => 'Nachbarschaft'
    ],
    'village' => [
        'name' => 'Dorf'
    ],
    'gsm_phone' => [
        'name' => 'GSM-Telefon'
    ],
    'land_phone' => [
        'name' => 'Festnetztelefon'
    ],
    'office_phone' => [
        'name' => 'Bürotelefon'
    ],
    'register_type' => [
        'name' => 'Registertyp'
    ],
    'identification_number' => [
        'name' => 'Identifikationsnummer'
    ],
    'adress_name' => [
        'name' => 'Adressname'
    ],
    'adress_first_name' => [
        'name' => 'Vorname'
    ],
    'adress_last_name' => [
        'name' => 'Nachname'
    ],
    'actions' => [
        'name' => 'Aktionen'
    ],
    'adress_content' => [
        'name' => 'Adressinhalt'
    ],
    'adress_post_code' => [
        'name' => 'Postleitzahl'
    ],
    'adress_gsm_phone' => [
        'name' => 'Gsm Telefon'
    ],
    'adress_land_phone' => [
        'name' => 'Festnetztelefon'
    ],
    'adress_country' => [
        'name' => 'Land'
    ],
    'adress_city' => [
        'name' => 'Stadt'
    ],
    'adress_district' => [
        'name' => 'Kreis'
    ],
    'adress_neighborhood' => [
        'name' => 'Nachbarschaft'
    ],
    'adress_village' => [
        'name' => 'Dorf'
    ],
    'messages' => [
        'name' => 'Mitteilungen'
    ],
    'user' => [
        'name' => 'Benutzer'
    ],





    /*Menu Button*/
    'profile' => [
        'name' => 'Profil'
    ],

    'create' => [
        'name' => 'Erstellen'
    ],
    'edit' => [
        'name' => 'Bearbeiten'
    ],
    'delete' => [
        'name' => 'Löschen'
    ],
    'list' => [
        'name' => 'Aufführen'
    ],
    'menu_address' => [
        'name' => 'Adresse'
    ],
    'menu_orders' => [
        'name' => 'Aufträge'
    ],
    'menu_favorites' => [
        'name' => 'Favoriten'
    ],
    'menu_archived_ads' => [
        'name' => 'Archivierte Anzeigen'
    ],
    'menu_pending_ads' => [
        'name' => 'Ausstehende Genehmigung'
    ],
    'menu_delete_account' => [
        'name' => 'Konto schließen'
    ],
    'menu_my_ads' => [
        'name' => 'Meine Anzeigen'
    ],
    'menu_fav_ads' => [
        'name' => 'Lieblingsanzeigen'
    ],
    'menu_hello_msg' => [
        'name' => 'Hallo'
    ],
    'menu_last_msg' => [
        'name' => 'Sie haben sich zuletzt um angemeldet'
    ],
    'menu_packages' => [
        'name' => 'Meine Pakete'
    ],
    'menu_adv_packages' => [
        'name' => 'Anzeigenpakete'
    ],
    'menu_time_packages' => [
        'name' => 'Zeitpakete'
    ],
    'profile_details' => [
        'name' => 'Profildetails'
    ],
    'profile_photo' => [
        'name' => 'Profilfoto'
    ],
    'adv_listing_banner' => [
        'name' => 'Banner der Anzeigenliste'
    ],
    'approve' => [
        'name' => 'Genehmigen'
    ],
    'approved' => [
        'name' => 'Genehmigt'
    ],
    'pending' => [
        'name' => 'steht aus'
    ],
    'passive' => [
        'name' => 'Passiv'
    ],
    'pending_admin' => [
        'name' => 'steht aus'
    ],
    'menu_messages' => [
        'name' => 'Mitteilungen'
    ],
    'menu_my_purchase' => [
        'name' => 'Mein Einkauf'
    ],
    'menu_my_sales' => [
        'name' => 'Meine Verkäufe'
    ],


    /* Right Dock*/
    'right_secure_trading_subject' => [
        'name' => 'Sicherer Handel'
    ],
    'right_secure_trading_msg' => [
        'name' => 'Lorem ipsum dolor sitzen amet, consectetur adipisicing elit'
    ],
    'right_support_subject' => [
        'name' => '24/7 Support'
    ],
    'right_support_msg' => [
        'name' => 'Lorem ipsum dolor sitzen amet, consectetur adipisicing elit'
    ],
    'right_easy_trading_subject' => [
        'name' => 'Einfacher Handel'
    ],
    'right_easy_trading_msg' => [
        'name' => 'Lorem ipsum dolor sitzen amet, consectetur adipisicing elit'
    ],
    'right_need_help_subject' => [
        'name' => 'Brauchen Sie Hilfe?'
    ],
    'right_need_help_msg' => [
        'name' => 'Rufen Sie an'
    ],
    'disable_account' => [
        'name' => 'Konto deaktivieren'
    ],
    'disable_account_msg' => [
        'name' => 'Ihr Konto wird deaktiviert.
                                    Dieser Vorgang kann nicht rückgängig gemacht werden'
    ],

    'message_title' => 'Nachrichtentitel',
    'owner_name' => 'Botschaft',

    'favorites' => "Favoriten",
    'fav_advs' => 'Lieblingsanzeigen',
    'fav_sellers' => "Lieblingsverkäufer",
    'fav_searches' => "Lieblingssuche",
    'posted_on' => "Veröffentlicht am",
    'message_details' => "Nachrichtendetails",
    'adv_no' => 'Anzeigen-Nr',
    'bill_address' => [
        'name' => 'Rechnungsadresse',
    ],
    'delivery_address' => [
        'name' => 'Lieferadresse',
    ],
    'order_total' => [
        'name' => 'Gesamt',
    ],
    'order_date' => [
        'name' => 'Bestelldatum',
    ],
    'order_no' => [
        'name' => 'Auftragsnummer',
    ],
    'order_detail' => [
        'name' => 'Detail anzeigen',
    ],
    /*Detail Page Order*/
    'detail' => [
        'name' => 'Detail',
    ],
    'back' => [
        'name' => 'Zurück',
    ],
    'image' => [
        'name' => 'Bild',
    ],
    'subject' => [
        'name' => 'Gegenstand',
    ],
    'price' => [
        'name' => 'Preis',
    ],
    'piece' => [
        'name' => 'Stück',
    ],
    'commission' => [
        'name' => 'Kommission',
    ],
    'total' => [
        'name' => 'Gesamt',
    ],
    'sub_total' => [
        'name' => 'Zwischensumme',
    ],
    'sale' => [
        'name' => 'Verkauf',
    ],
    'awaiting_tracking_number' => [
        'name' => 'Warten auf Tracking-Nummer',
    ],
    'awaiting_payment_approval' => [
        'name' => 'Warten auf Zahlungsgenehmigung',
    ],
    'paid' => [
        'name' => 'Bezahlt',
    ],
    'cancelled' => [
        'name' => 'Abgesagt',
    ],
    'waiting' => [
        'name' => 'Warten',
    ],
    'awaiting_dispatch' => [
        'name' => 'Warten auf Versand',
    ],
    'shipped' => [
        'name' => 'Versand',
    ],
    'delivered' => [
        'name' => 'Geliefert',
    ],
    'tracking_number' => [
        'name' => 'Auftragsnummer, Frachtnummer, Sendungscode',
    ],
    'was_delivered' => [
        'name' => 'wurde geliefert',
    ],
    'not_delivered' => [
        'name' => 'nicht zugestellt',
    ],
    'show_order_msg1' => [
        'name' => 'Das Transaktionsergebnis wird per Mail benachrichtigt.',
    ],
    'show_order_msg2' => [
        'name' => 'Eine Rückerstattung erfolgt, wenn das Produkt nicht innerhalb einer Woche geliefert wird.',
    ],
    'show_order_msg3' => [
        'name' => 'Das Ergebnis der Transaktion wurde an Ihre registrierte E-Mail-Adresse gesendet.',
    ],

    'awaiting_payment' => [
        'name' => 'Warten auf Zahlung',
    ],
    'please_entered_tracking_number' => [
        'name' => 'Bitte geben Sie die Tracking-Nummer ein',
    ],
    'transport_days' => [
        'name' => 'Transporttage',
    ],
    'product_not_delivered' => [
        'name' => 'Produkt nicht geliefert',
    ],
    'status' => [
        'name' => 'Status',
    ],
    'cancel_sale' => [
        'name' => 'Verkauf abbrechen',
    ],
    'send_again' => [
        'name' => 'Ich werde wieder senden',
    ],
    'content' => [
        'name' => 'Inhalt',
    ],
    'preferences_settings' => [
        'name' => 'Einstellungen Einstellungen',
    ],


    'corporate_settings' => [
        'name' => 'Unternehmenseinstellungen',
    ],
    'password' => [
        'name' => 'Passwort',
    ],
    'new_password' => [
        'name' => 'Neues Kennwort',
    ],
    're_new_password' => [
        'name' => 'Bestätige das Passwort',
    ],
    'confirm_password_input' => [
        'name' => 'Ich bestätige meine Passwortänderung',
    ],
    'create_address' => [
        'name' => 'Adresse erstellen',
    ],
    'edit_address' => [
        'name' => 'Adresse bearbeiten',
    ],
    'balance_limit' => [
        'name' => 'Balance Limit',
    ],
    'active_ads' => [
        'name' => 'Aktive Anzeigen',
    ],
    'subscriptions' => [
        'name' => 'Abonnements',
    ],
    'go_subscriptions_page' => [
        'name' => 'Gehen Sie zur Abonnementseite',
    ],
    'buy_package' => [
        'name' => 'Paket kaufen',
    ],
    'expired_date' => [
        'name' => 'Abgelaufenes Datum',
    ],
    'category' => [
        'name' => 'Kategorie',
    ],
    'ad_limit' => [
        'name' => 'Anzeigenlimit',
    ],
    'publish_time' => [
        'name' => 'Zeit veröffentlichen',
    ],
    'all_categories' => [
        'name' => 'Alle Kategorien',
    ],
    'day' => [
        'name' => 'Tag',
    ],

    'individual' => [
        'name' => 'Individuell',
    ],
    'corporate' => [
        'name' => 'Unternehmen',
    ],

    'details' => [
        'name' => 'Einzelheiten'
    ],

];
