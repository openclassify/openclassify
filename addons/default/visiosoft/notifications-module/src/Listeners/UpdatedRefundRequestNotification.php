...
class UpdatedRefundRequestNotification
{
    private $template;
    private $user;
    ...
    public function handle(UpdatedRefundRequest $event)
    {
        ...
        if ($user = $this->user->find($refund->created_by_id)) {
            ...
            $mail_params = [
                ...
                'ad_name' => $adv->name,
                'refund_message' => $refund->refund_message,
            ];

            //Send Admins
            foreach ($admins as $admin) {
                if ($template = $this->template->findBySlug('refund_status_changed_for_admin')) {
                    $admin->notify(new MailTemplate($template->getTemplateForArray($mail_params)));
                }
            }

            //Send Seller
            if ($template = $this->template->findBySlug('refund_status_changed_for_seller')) {
                $seller->notify(new MailTemplate($template->getTemplateForArray($mail_params)));
            }

            //Send User
            if ($template = $this->template->findBySlug('refund_status_changed_for_user')) {
