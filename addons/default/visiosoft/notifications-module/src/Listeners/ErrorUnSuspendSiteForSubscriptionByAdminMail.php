...
class ErrorUnSuspendSiteForSubscriptionByAdminMail
{
    ...
    public function handle(ErrorUnSuspendSiteForSubscription $event)
    {
        $site = $event->getSite();

        if ($user = $site->assign) {
            $template = $this->template->findBySlug(str_slug('Error UnSuspend Site For Subscription By Admin', '_'));
