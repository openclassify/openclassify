...
class CalculateClassifiedsCount
{
    use DispatchesJobs;
    ...
    {
        $classifiedRepository = app(ClassifiedRepositoryInterface::class);
        if (!empty($level)) {
            $count = $classifiedRepository->getClassifiedsCountByCategory($category_id, $level);
