...
class ErrorSuspendSiteForWebhookMail
{
    ...
    public function handle(ErrorSuspendSiteForWebhook $event)
    {
        $site = $event->getSite();

        if ($user = $site->assign) {
            $template = $this->template->findBySlug(str_slug('Error Suspend Site For Webhook', '_'));
